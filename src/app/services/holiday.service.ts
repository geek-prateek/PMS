...
export class HolidayService implements OnInit{
    ...
    calculateBusinessDays(startDate: Date, endDate: Date): number {
        ...
        const millisecondsPerDay: number = 86400 * 1000; // Day in milliseconds
        startDate.setHours(0,0,0,1);  // Start just after midnight
        endDate.setHours(23,59,59,999);  // End just before midnight
        const diff: number = endDate.getTime() - startDate.getTime();  // Milliseconds between datetime objects    
        let days: number = Math.ceil(diff / millisecondsPerDay);
